...
- Risk: Every valid email request creates a `users` row and a `magic_login_tokens` row, with no cleanup command for expired or used tokens.
...
- Current mitigation: Request route has `throttle:5,15`, tokens are hashed, expire after 30 minutes, and can be used once.
