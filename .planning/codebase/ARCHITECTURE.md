...
3. `app/Http/Controllers/Auth/MagicLinkController.php` validates the email and Turnstile token, verifies Turnstile server-side, then creates or finds `app/Models/User.php`, stores a hashed token in `app/Models/MagicLoginToken.php`, creates a relative signed URL, and sends `app/Mail/MagicLinkMail.php`.
...
6. `MagicLinkController::consume` verifies the token hash, marks it used, logs in through the web guard, regenerates the session, and redirects to `today.show`.
